...
impl DaggerWasm {
    ...
    /// Load a network from a directory path
    ...
    #[wasm_bindgen]
    pub fn query(&self, network_path: &str, query_str: &str) -> Result<String, JsValue> {
        // Load network
        ...
        let config_path = std::path::Path::new(network_path).join("config.toml");
        ...
        let result = executor.execute(&query_path)
    ...
    #[wasm_bindgen]
    pub fn get_nodes(&self, network_path: &str, node_type: Option<String>) -> Result<String, JsValue> {
        let (network, _validation) = parser::load_network_from_directory(network_path)
            .map_err(|e| JsValue::from_str(&format!("Failed to load network: {}", e)))?;

        let nodes: Vec<_> = network.nodes
            .iter()
            .filter(|n| {
                if let Some(ref filter_type) = node_type {
                    match n {
                        ...
                        parser::models::NodeData::GeographicAnchor(_) => filter_type == "geographicAnchorNode",
                        parser::models::NodeData::GeographicWindow(_) => filter_type == "geographicWindowNode",
                    }
                } else {
                    true
                }
    ...
    #[wasm_bindgen]
    pub fn get_edges(&self, network_path: &str, source: Option<String>, target: Option<String>) -> Result<String, JsValue> {
        let (network, _validation) = parser::load_network_from_directory(network_path)
            .map_err(|e| JsValue::from_str(&format!("Failed to load network: {}", e)))?;

        let edges: Vec<_> = network.edges
            .iter()
            .filter(|e| {
    ...
    pub fn get_schema_versions(&self, schemas_dir: &str) -> Result<String, JsValue> {
        let mut registry = schema::registry::SchemaRegistry::new(std::path::PathBuf::from(schemas_dir));
        ...
        if schemas_path.exists() {
            let entries = std::fs::read_dir(schemas_path)
                .map_err(|e| JsValue::from_str(&format!("Failed to read schemas directory: {}", e)))?;
            
            for entry in entries {
                let entry = entry
                    .map_err(|e| JsValue::from_str(&format!("Failed to read directory entry: {}", e)))?;
                let path = entry.path();
    ...
    #[wasm_bindgen]
    pub fn validate_block(&self, schemas_dir: &str, version: &str, block_type: &str, block_json: &str) -> Result<String, JsValue> {
        let mut registry = schema::registry::SchemaRegistry::new(std::path::PathBuf::from(schemas_dir));
        ...
        let issues: Vec<_> = result.issues.iter().map(|issue| {
            serde_json::json!({
