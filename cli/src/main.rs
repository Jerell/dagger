...
fn main() {
    ...
    match cli.command {
        Commands::Export { path, output } => {
            match export_network(&path, output.as_deref()) {
                Ok(_) => {}
                Err(e) => {
                    eprintln!("Error: {}", e);
                    std::process::exit(1);
        ...
        Commands::Query { path, query } => {
            match query_network(&path, &query) {
                Ok(_) => {}
                Err(e) => {
                    eprintln!("Error: {}", e);
                    std::process::exit(1);
...
fn list_nodes(path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for node in &network.nodes {
        let base = node.base();
        println!("  - {} ({}) at ({}, {})", base.id, base.label_display(), base.position.x, base.position.y);
    }
    
    println!("\nEdges ({}):", network.edges.len());
    for edge in &network.edges {
        println!("  - {} -> {} (weight: {})", edge.source, edge.target, edge.data.weight);
    }
