...
import { loadNetwork, getNetworkNodes, getNetworkEdges } from '../services/network';
...
networkRoutes.get('/', async (c) => {
  ...
  try {
    ...
    return c.json(
      { error: 'Failed to load network', message: error instanceof Error ? error.message : String(error) },
      500
    );
  }
});
...
networkRoutes.get('/nodes', async (c) => {
  ...
  try {
    ...
    return c.json(
      { error: 'Failed to load nodes', message: error instanceof Error ? error.message : String(error) },
      500
    );
  }
});
...
networkRoutes.get('/edges', async (c) => {
  ...
  try {
    ...
    return c.json(
      { error: 'Failed to load edges', message: error instanceof Error ? error.message : String(error) },
      500
    );
  }
});
