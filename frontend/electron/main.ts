...
type OperationsServerConfig = {
  costingUrl: string | null;
  modellingUrl: string | null;
};
...
const preloadPath = join(frontendRoot, "dist-electron", "preload.js");
const rendererDevUrl = process.env.ELECTRON_RENDERER_URL ?? "http://127.0.0.1:3000";
const rendererProdPath = join(frontendRoot, ".output", "public", "index.html");
...
async function startBackend(backendPath = backendRoot): Promise<string> {
  if (await isBackendHealthy()) {
...
function registerIpcHandlers(): void {
  ipcMain.handle("desktop:start-local-server", async (_event, backendPath?: string) => {
    return startBackend(backendPath ?? backendRoot);
  });
  ...
  ipcMain.handle("desktop:read-network-directory", async (_event, directoryPath: string) => {
    ...
    const files = await Promise.all(
      entries
        ...
        .map(async (entry): Promise<NetworkFile> => {
  ...
  ipcMain.handle("desktop:get-operations-config", async (): Promise<OperationsServerConfig> => {
    return {
      costingUrl: process.env.COSTING_SERVER_URL ?? "http://localhost:8080",
      modellingUrl: process.env.MODELLING_SERVER_URL ?? "http://localhost:4001",
    };
  });
